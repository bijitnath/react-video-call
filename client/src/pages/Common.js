...
import calling from '../Icons/calling.gif'
    ...
    useEffect(() => {
        ...
        socket.current.on("socket_id", (id) => {
            ...
            if (callFrom != null) {
                ...
                setText(callFromName + " is trying to call you.. please wait to accept/reject the call.")
                var myID = queryParams.get('myID');
                socket.current.emit("socket_id_reciever", { callFrom: callFrom, myID: myID, socketID: id })
            }
            else {
                var toID = queryParams.get('toID');
                var myID = queryParams.get('myID');
                var callTo = queryParams.get('callTo');
                setText("Calling " + callTo + ", Please wait for the user to come online.")
                setCallTo(callTo);
                if (toID != null) {
                    setReceiverID(toID)
                    socket.current.on("socket_id_reciever", (data) => {
                        if (data.callFrom == myID) {
                            if (data.myID == toID) {
                                callPeer(data.socketID, id)
                            }
                        }
